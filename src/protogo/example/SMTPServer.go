...
func (m *Mail) OnRequest(line telnet.Line) telnet.Response {
    ...
    switch {
        case m.who == "" :  return m.heloStep(cmd)
        case m.from == "" : return m.mailStep(cmd)
    }
...
func (m *Mail) send() {
    fmt.Println("Accepted mail")
    fmt.Println("-------------")
    fmt.Println("From : ", m.from)
    fmt.Println("To : ", m.recipients)
...
func (m *Mail) mailStep(cmd *cmdLine) telnet.Response {
    ...
    switch {
        case len(cmd.Command.Args) > 2      : return syntaxError("Too many arguments given")
        case len(cmd.Command.Args) != 2     : return syntaxError("Not enough arguments given")
...
func parseCmd(line telnet.Line) *cmdLine {
    ...
    cmd := line.AsCommand()
    switch(cmd.Name) {
        case "noop" : return &cmdLine{cmd, NOOP}
        case "helo" : return &cmdLine{cmd, HELO}
        case "mail" : return &cmdLine{cmd, MAIL}
        case "rcpt" : return &cmdLine{cmd, RCPT}
        case "data" : return &cmdLine{cmd, DATA}
        case "rset" : return &cmdLine{cmd, RSET}
        case "send" : return &cmdLine{cmd, SEND}

        case "quit", "bye", "exit" : return &cmdLine{cmd, QUIT}
    }
