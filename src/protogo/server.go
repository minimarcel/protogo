...
func (srv *Server) serve() {
	...
	log.Println("Start serving requests")
	defer srv.Stop()
	...
	for {
		...
		if err != nil {
			log.Println("Unable to accept a connection : ", err)
			continue
		}
...
func (srv *Server) wrappConn(conn net.Conn) *serverConn {
	...
	if len(srv.conns) >= srv.maxConns {
		log.Printf("Refuse connection from %s, too much opened connections\n", conn.RemoteAddr().String())
		conn.Close()
		return nil
	}
...
func (conn *serverConn) Close() error {
	...
	log.Println("Connection closed for ", conn.Conn.RemoteAddr().String())
	// release ourselves
